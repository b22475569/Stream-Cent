...
' History server port updated: 8178
' ============================================================
Option Explicit
...
' -- Start HTTP server on Port 8078 (skip if already running)
Dim chk1
chk1 = WshShell.Run("cmd /c netstat -ano | find "":8078"" | find ""LISTENING"" > nul 2>&1", 0, True)
If chk1 <> 0 Then
...
Dim chk2
chk2 = WshShell.Run("cmd /c netstat -ano | find "":8178"" | find ""LISTENING"" > nul 2>&1", 0, True)
If chk2 <> 0 Then
